...
function submitFile(relativePath, newContent) {
  return axios
    .post(`/submit`, { relativePath, newContent })
    .then((res) => res.data);
}

// 文件删除数组
function deleteFiles(files) {
  return axios.post("/delete", { files }).then((res) => res.data);
}

// 文件重命名
function renameFile(relativePath, originName, newName) {
  return axios
    .post(`/rename`, { relativePath, originName, newName })
    .then((res) => res.data);
}
...
function addFile(relativePath, fileName) {
  return axios
    .post(`/createFile`, { relativePath, fileName })
    .then((res) => res.data);
}
...
function uploadFile(progress, formData) {
  ...
  return axios.post("/upload", formData, config).then((res) => res.data);
}

// 文件复制
function copyFile(fileName, srcRelativePath, targetRelativePath) {
  return axios
    .post("/copyFile", { fileName, srcRelativePath, targetRelativePath })
    .then((res) => res.data);
}
...
function addDir(relativePath, folderName) {
  return axios
    .post(`/createFolder`, { relativePath, folderName })
    .then((res) => res.data);
}
...
function login(config) {
  return axios.post("/login", { ...config }).then((res) => res.data);
}
...
// 判断自动登录
function autoLogin() {
  return axios.get("/autoLogin").then((res) => res.data);
}

// 注册请求
function register(config) {
  return axios.post("/register", { ...config }).then((res) => res.data);
}
